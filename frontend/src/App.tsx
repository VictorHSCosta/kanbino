  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-indigo-950">
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="max-w-4xl mx-auto bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-xl shadow-indigo-500/20 overflow-hidden">
          <div className="p-8">
            ...
            {status && (
              <div className="mt-6 p-4 bg-slate-700/50 rounded-lg">
                <h2 className="text-lg font-semibold text-slate-200 mb-3">Informações do Sistema</h2>
            ...
            {data && (
              <div className="mt-6">
                <h2 className="text-lg font-semibold text-slate-200 mb-3">Dados do Backend</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {data.items.map((item) => (
                    <div key={item.id} className="p-3 bg-indigo-900/40 rounded-lg border border-indigo-700/50 hover:scale-105 transition-transform cursor-pointer">
                      <p className="font-semibold text-indigo-200">{item.name}</p>
                      <p className="text-sm text-indigo-300 capitalize">{item.type}</p>
                    </div>
                  ))}
                </div>
              </div>
